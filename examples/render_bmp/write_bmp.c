               ...
               int width, int height)
{
    ...
    /* Bitmap file header */
    fwrite("BM", 2, 1, f);
    write_32b(f, 14 + 12 + width * height); /* File length */
    write_16b(f, 0); /* Reserved */
    write_16b(f, 0); /* Reserved */
    write_32b(f, 14 + 12); /* Offset to pixel data */
    ...
    write_16b(f, 24); /* Bits per pixel */
    
    /* Pixel data */
    for (i = 0; i < height; i++)
    {
        fwrite(data + width * (height - 1 - i) * 3, width * 3, 1, f);
    }
